...
def coco_single_class_labels(path='../coco/labels/train2014/', label_class=43):
    ...
    for file in tqdm(sorted(glob.glob('%s/*.*' % path))):
        ...
        if any(i):
            ...
            with open(os.path.join(settings.MEDIA_ROOT, 'new/images.txt'), 'a') as f:  # add image to dataset list
                f.write(img_file + '\n')
            with open(os.path.join(settings.MEDIA_ROOT, 'new/labels/' + Path(file).name), 'a') as f:  # write label
                for l in labels[i]:
...
def print_mutation(hyp, results, bucket=''):
    ...
    with open(os.path.join(settings.MEDIA_ROOT, 'evolve.txt'), 'a') as f:  # append result
        f.write(c + b + '\n')
