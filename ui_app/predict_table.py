...
def norm_pdf_page(pdf_file, pg):
    pdf_doc = PdfFileReader(open(os.path.join(settings.MEDIA_ROOT, pdf_file), "rb"))
...
def pdf_page2img(pdf_file, pg, save_image=True):
    img_page = convert_from_path(os.path.join(settings.MEDIA_ROOT, pdf_file), first_page=pg, last_page=pg)[0]
    if save_image:
...
# %%
def detect_tables(pdf_file, pg, client_pn, invoice_pn):
    ...
    img = pdf_page2img(os.path.join(settings.MEDIA_ROOT, pdf_file), pg)
    opt = parameters(img_path)
    output_detect = detectTable(opt)
    output = outpout_yolo(output_detect)

    os.remove(os.path.join(settings.MEDIA_ROOT, img_path))
    os.rmdir("outputs")
    ...
    output_camelot = camelot.read_pdf(
        filepath=os.path.join(settings.MEDIA_ROOT, pdf_file), pages=str(pg), flavor="stream", table_areas=interesting_areas
    )
    print("Output_camelot", output_camelot)
    print("output camelot", output_camelot.n)
    if output_camelot.n == 1:
        return output_camelot[0].df
    # print(output_camelot.n == 1)
    # print("output_camelot[0]", output_camelot[0].df)
    # print("__________________________________________")
    # print("output_camelot[1]", output_camelot[1].df)
    # output_camelot[1] = [output_camelot[1], 0]
    ...
    elif client_pn != invoice_pn:
        return output_camelot[0].df

    return output_camelot[1].df
